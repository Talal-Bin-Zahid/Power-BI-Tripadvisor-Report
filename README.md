# 🏨 Power-BI-Tripadvisor-Report
<img width="970" height="548" alt="TripAdvisor Report" src="https://github.com/user-attachments/assets/7558e641-b43a-4483-b9dd-1f65e9518232" />

📌 Project Title & Overview

TripAdvisor Report | User Demographics, Hotel Performance & Seasonal Trends Analysis

This Power BI dashboard analyzes TripAdvisor user data to uncover insights into traveler demographics, hotel performance, capacity distribution, and seasonal travel behavior. The report is designed to help hospitality stakeholders understand customer segments, identify top-performing hotels, and optimize strategies based on demand patterns.

 ---

📊 Key Metrics & Insights
👥 User Demographics

Users by Continent

North America: 295 users (highest representation)

Europe: 118 users

Oceania: 41 users

Asia: 36 users

Africa: 7 users

South America: 7 users

Traveler Type Distribution

Couples: 214 users (largest segment)

Families: 110 users

Friends: 82 users

Business: 74 users

Solo: 24 users

 ---
 
⭐ Hotel Ratings & Performance

Hotels by Star Rating

5-Star Hotels: 8

4-Star Hotels: 5

3-Star Hotels: 4

2-Star Hotels: 3

1-Star Hotels: 1

 ---
 
Top 10 Hotels by Score

Wynn Las Vegas: ⭐ 4.63 (highest rated)

The Venetian: 4.58

Encore at Wynn: 4.54

Marriott’s Grand: 4.54

The Palazzo Resort: 4.38

Trump International: 4.38

Wyndham Grand: 4.38

The Cosmopolitan: 4.25

Bellagio Las Vegas: 4.21

Tuscany Las Vegas: 4.21

 ---

📅 Seasonality Trends

Users by Period of Stay

Mar–May: 25.4%

Jun–Aug: 25%

Sep–Nov: 25%

Dec–Feb: 24.6%

 ---

📌 User stays are evenly distributed across seasons, indicating consistent year-round travel demand.

🏢 Hotel Capacity Insights

Top 10 Hotels by Number of Rooms

The Venetian: 4.03K rooms (largest capacity)

Excalibur Hotel: 3.98K

Bellagio Las Vegas: 3.93K

Caesars Palace: 3.77K

The Palazzo: 3.35K

Monte Carlo: 3.03K

The Cosmopolitan: 3.00K

Paris Las Vegas: 2.96K

Treasure Island: 2.92K

Circus Circus: 2.88K

 ---

📈 Visualizations Used

Bar Charts
Used for user distribution by continent, traveler type, and hotel capacity comparisons.

Donut Charts
Represent hotel ratings, seasonal stays, and hotel performance rankings.

Column Charts
Show hotel star distribution to highlight quality segmentation.

Each visualization is interactive, allowing users to quickly compare categories and identify trends.

 ---

🛠️ Technical Stack

Power BI Desktop – Data modeling, visualization, and report creation

Power Query – Data extraction, cleaning, and transformation (ETL)

DAX (Data Analysis Expressions) – Calculated measures and aggregations

---

📌 Conclusions & Business Value

This dashboard provides actionable insights for hospitality and travel businesses:

Couples represent the largest traveler segment, indicating strong potential for romantic and leisure-focused packages.

North America dominates the user base, making it a key target market.

High-capacity hotels like The Venetian play a major role in accommodating demand.

Even seasonal distribution suggests opportunities for consistent pricing and year-round promotions.

---
